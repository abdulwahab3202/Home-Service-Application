    ...
    <div className="group bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden border border-slate-100 dark:border-slate-800 flex flex-col h-full">
      ...
      <div className="px-5 pt-4 flex justify-between items-center">
          ...
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border uppercase tracking-wide ${
            job.status === 'PENDING' ? 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800' : 
            job.status === 'ASSIGNED' || job.status === 'IN_PROGRESS' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800' : 
...
const WorkerDashboard = () => {
  ...
  const handleRevoke = async () => {
    ...
    if (result.isConfirmed) { try { await revokeJob(); } catch (e) {} }
  };
  ...
  const handleAccept = async (id) => {
    const result = await Swal.fire({
      title: 'Accept Job?',
      text: "You will be assigned this request.",
      icon: 'question',
      showCancelButton: true,
  ...
  if (isLoading && !showOtpModal) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 flex flex-col items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 transition-colors duration-500">
      ...
      {showOtpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate-in zoom-in-95 border border-slate-100 dark:border-slate-800">
            <button onClick={() => setShowOtpModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600"><X size={24} /></button>
            <div className="text-center mb-6">
              ...
              <p className="text-slate-500 mt-2 text-sm">Enter code sent to <span className="font-semibold text-slate-700">{activeJob?.customerEmail || "Customer"}</span></p>
            </div>
            <div className="flex justify-center gap-3 mb-8">
              {otp.map((digit, index) => (
                <input key={index} ref={(el) => (inputRefs.current[index] = el)} type="text" maxLength="1" value={digit} onChange={(e) => handleOtpChange(index, e.target.value)} onKeyDown={(e) => handleKeyDown(index, e)} className="w-14 h-14 border-2 border-slate-200 rounded-xl text-center text-2xl font-bold focus:border-indigo-500 outline-none" />
              ))}
            </div>
            <button onClick={handleSubmitOtp} disabled={isLoading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl shadow-lg flex items-center justify-center gap-2">
              {isLoading ? <Loader2 className="animate-spin" /> : 'Verify & Complete Job'}
            </button>
            <div className="mt-4 text-center">
                <button onClick={() => { setShowOtpModal(false); handleInitiateComplete(); }} className="text-xs text-indigo-600 hover:underline">Resend Code / Issues?</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-10">
            <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white">Worker Dashboard</h1>
            <p className="text-slate-500 mt-1">Manage assignments and accept new work.</p>
        </div>

        {activeJob && (
          <div className="mb-12">
             <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
              <Clock className="text-indigo-600" /> Current Assignment
            </h2>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                <WorkerJobCard job={activeJob} activeJob={activeJob} isActionable={false} onInitiateComplete={handleInitiateComplete} onRevoke={handleRevoke} onImageClick={setSelectedImage} />
            </div>
          </div>
        )}
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
            <AlertCircle className="text-orange-500" /> New Requests
          </h2>
          {filteredAvailableJobs.length === 0 ? (
            <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200">
               <div className="bg-slate-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"><Briefcase className="text-slate-400" size={32} /></div>
              <p className="text-slate-500 font-medium">No new jobs available right now.</p>
            </div>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredAvailableJobs.map((job) => (
                <WorkerJobCard key={job.id} job={job} activeJob={activeJob} isActionable={true} onAccept={handleAccept} onImageClick={setSelectedImage} />
              ))}
            </div>
          )}
        </div>
